...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:param name="include-base" as="xs:boolean" required="yes"/>
  ...
  <xsl:template match="/*" xmlns="http://www.w3.org/ns/xproc-step">
    ...
    <xsl:variable name="base" as="xs:string" select="xtlc:href-path($base-raw)"/>
    
    <c:archive>
      <xsl:for-each select="//c:file">
        <xsl:variable name="name-for-href" as="xs:string" select="xtlc:href-concat(ancestor-or-self::*/@name)"/>
        <xsl:variable name="name" as="xs:string">
          <xsl:choose>
            <xsl:when test="$include-base">
              <xsl:sequence select="$name-for-href"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:sequence select="xtlc:href-concat(subsequence(ancestor-or-self::*/@name, 2))"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <c:entry name="{$name}" href="{xtlc:href-add-encoding(xtlc:href-concat(($base, $name-for-href)))}"/>
      </xsl:for-each>
    </c:archive>
